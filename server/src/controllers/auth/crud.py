...
class CRUD:
    ...
    def create(self, email, username, password):
        ...
        # Validate Email
        if self.helpers.check_email(email) is not True:
        ...
        # Validate password
        if len(password) < 6:
            return json.dumps({'error': "password is too short,it must have atleast 8 characters"}, default=self.helpers.set_default, sort_keys=True, indent=4), HTTP_400_BAD_REQUEST
        ...
        if create_user is None:
            return json.dumps({"user created succesfully"}, default=self.helpers.set_default, sort_keys=True, indent=4), HTTP_201_CREATED
